...
```shell
curl -s https://raw.githubusercontent.com/h2020-sbd4nano/sbd-data-book/master/sparql/allGuidanceDocuments.rq | sed 's+<lang/>+en+' > allGuidanceDocuments.rq

curl -H "Accept: text/tab-separated-values" -G https://sbd4nanolandscape.rdf.bigcat-bioinformatics.org/sparql --data-urlencode query@allGuidanceDocuments.rq
```
